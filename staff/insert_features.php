...
<table class="table table-bordered border-dark mt-5">
    ...
    <tbody>
        <?php
        $user_id = get_user_id_staff();
        $pic = "SELECT * FROM `features` where user_id='$user_id'";
        ...
        if ($result_select) {
            if (mysqli_num_rows($result_select) > 0) {
                ...
                while ($row = mysqli_fetch_array($result_select)) {
                    ...
                    <td>$number</td>
                    <td>$feature_title</td>
                    <td><a href='staff.php?edit_features=$feature_id' class='text-dark'><i class='fas fa-edit text-dark'></i></td>
                    <td><a href='staff.php?delete_features=$feature_id' class='text-dark'><i class='fas fa-trash'></i></a></td>
                    </tr>";
                }
            } else {
