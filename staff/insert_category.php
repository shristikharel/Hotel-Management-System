...
if (isset($_POST['insert_category'])) {
    ...
    if (!$category_exists) {
        ...
        if ($result) {
            ...
            echo "<script>window.open('staff.php?view_categories', '_self')</script>";
        } else {
...
<table class="table table-bordered border-dark mt-5">
    ...
    <tbody>
        <?php
        $select_categories = "SELECT * FROM `categories` where user_id='$user_id'";
        ...
        if ($result_categories) {
            if (mysqli_num_rows($result_categories) > 0) {
                ...
                while ($row = mysqli_fetch_array($result_categories)) {
                    ...
                    <td>$number</td>
                    <td>$category_name</td>
                    <td><a href='staff.php?edit_category=$category_id' class='text-dark'><i class='fas fa-edit text-dark'></i></td>
                    <td><a href='staff.php?delete_category=$category_id' class='text-dark'><i class='fas fa-trash'></i></a></td>
                    </tr>";
                }
            } else {
