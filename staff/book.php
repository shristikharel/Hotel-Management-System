...
<table class="table table-bordered mt-5">
    ...
    where book_details.room_id=rooms.room_id AND book_details.status='pending' AND rooms.user_id='$user_id'";
$result = mysqli_query($con, $query);
    ...
    <td>
        <form action="staff.php?view_bookings" method="POST">
...
<table class="table table-bordered mt-5">
    ...
    DATEDIFF(checkoutdate, checkindate) * price as room_price, book_details.status
        from `book_details`, `rooms` 
        where book_details.room_id=rooms.room_id AND book_details.status='approved'  AND rooms.user_id='$user_id'";
$result = mysqli_query($con, $query);
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_array($result)) { ?>
    <tbody class="text-center">
...
<table class="table table-bordered mt-5">
    ...
    DATEDIFF(checkoutdate, checkindate) * price as room_price, book_details.status
        from `book_details`, `rooms` 
        where book_details.room_id=rooms.room_id AND book_details.status='cancelled'  AND rooms.user_id='$user_id'";
$result = mysqli_query($con, $query);
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_array($result)) { ?>
    <tbody class="text-center">
